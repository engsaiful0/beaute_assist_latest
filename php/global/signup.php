...
if(isset($_GET['confirm']))
{
    ...
    {
        $pdo = ORM::get_db();
        $query = "UPDATE `".$config['db']['pre']."user` SET `email_verify` = '1', `confirm` = '' WHERE id='".mysqli_real_escape_string($mysqli,$_GET['user'])."' AND confirm='".mysqli_real_escape_string($mysqli,$_GET['confirm'])."' LIMIT 1 ";
...
if(isset($_POST["submit"])) {
    ...
    if($errors == 0) {
        ...
        if($_POST["user-type"] == 1){
            $insert_user->user_type = 'beautician';
        }else{
            $insert_user->user_type = 'user';//employer
        }
       ...
       // create_user_session($loggedin['id'],$loggedin['username'],$loggedin['password'], $loggedin['user_type']);
        ...
        message(__("Welcome"),__("Welcome to <b>Beautyassist</b>. Activation link has been sent to your email. Please activate your account. Thanks"),'',false);
        exit;
    }
}
