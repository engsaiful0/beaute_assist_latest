...
if (!empty($result1)) {
    ...
    foreach ($result1 as $info) {
        ...
        $freelancers[$info['id']]['address'] = strtolower($info['address']);
...
// custom settings
    ...
    if ($plan['status']) {
        ...
        $sub_types[$plan['id']]['service_commission_to_admin'] = $settings['service_commission_to_admin'];
        $sub_types[$plan['id']]['skills_limit'] = $settings['skills_limit'];
        $sub_types[$plan['id']]['service_posting_limit'] = $settings['service_posting_limit'];
        $sub_types[$plan['id']]['service_expire_duration'] = $settings['service_expire_duration'];
      
        $sub_types[$plan['id']]['number_of_meeting_schedules'] = $settings['number_of_meeting_schedules'];
        $sub_types[$plan['id']]['featured_badge_duration'] = $settings['featured_badge_duration'];
        $sub_types[$plan['id']]['urgent_badge_duration'] = $settings['urgent_badge_duration'];
    ...
    foreach ($rows as $plan) {
        ...
        $sub_types[$plan['id']]['service_commission_to_admin'] = $plan['service_commission_to_admin'];
        $sub_types[$plan['id']]['skills_limit'] = ($plan['skills_limit'] == "999") ? __('Unlimited') : $plan['skills_limit'];
        $sub_types[$plan['id']]['service_posting_limit'] = ($plan['service_posting_limit'] == "999") ? __('Unlimited') : $plan['service_posting_limit'];
        $sub_types[$plan['id']]['service_expire_duration'] = ($plan['service_expire_duration'] == "999") ? __('Unlimited') : $plan['service_expire_duration'];
        $sub_types[$plan['id']]['number_of_meeting_schedules'] = ($plan['number_of_meeting_schedules'] == "999") ? __('Unlimited') : $plan['number_of_meeting_schedules'];
        $sub_types[$plan['id']]['featured_badge_duration'] = ($plan['featured_badge_duration'] == "999") ? __('Unlimited') : $plan['featured_badge_duration'];
        $sub_types[$plan['id']]['urgent_badge_duration'] = ($plan['urgent_badge_duration'] == "999") ? __('Unlimited') : $plan['urgent_badge_duration'];
...
$total_users = $count = ORM::for_table($config['db']['pre'].'user')->where('user_type','user')->count();
$total_beauticians = $count = ORM::for_table($config['db']['pre'].'user')->where('user_type','beautician')->count();
$total_services = $count = ORM::for_table($config['db']['pre'].'post')->count();
...
HtmlTemplate::display($home_page, array(
    ...
    'total_beauticians' => $total_beauticians,
    'popular' => $popular,
