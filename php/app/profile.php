...
if (isset($_GET['username'])) {
    ...
    if (is_array($get_userdata)) {
        ...
        if ($get_userdata['user_type'] == 'beautician') {
        ...
        if ($get_userdata['user_type'] == 'user') { //employer
        ...
        if ($get_userdata['user_type'] == 'beautician') {
            $result = ORM::for_table($config['db']['pre'] . 'gallery')
                ->where('beautician_id', $user_id)->find_many();
            $beautician_id = $user_id;
            foreach ($result as $info) {
                $gallery_images[$info['id']]['image'] = $info['image'];
            }
        }
        ...
        HtmlTemplate::display('profile', array(
            ...
            'item_link' => $item_link,
            'fullname' => $user_name,
            'userid' => $beautician_id,
            'user_id' => $employer_id, //employer
