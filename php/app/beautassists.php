...
$total = mysqli_num_rows(mysqli_query($mysqli, "SELECT 1 FROM `".$config['db']['pre']."user` u where u.status = '1' AND u.user_type = 'beautician' $where"));

$query = "SELECT u.* FROM `".$config['db']['pre']."user` u
     where u.status = '1' AND u.user_type = 'beautician' $where ORDER BY $order_by LIMIT ".($page_number-1)*$limit.",$limit";
...
if (mysqli_num_rows($result) > 0) {
    ...
    while($info = mysqli_fetch_assoc($result)) {
        ...
        $hourly_rate = get_user_option($info['id'],'hourly_rate','0');
        $hourly_rate = ($hourly_rate)? $hourly_rate : '';

        $skills_data = get_user_option($info['id'],'skills');
        if ($skills_data != "") {
        ...
        $items[$info['id']]['hourly_rate'] = $hourly_rate;
        $items[$info['id']]['skills'] = $skills;
        ...
        $items[$info['id']]['address'] = $info['address'];
...
if(count($_GET) >= 1){
    ...
    $Pagelink .= "?".$get;
    $pagging = pagenav($total,$page_number,$limit,$link['BEAUTASSIST'].$Pagelink,1);
}else{
    $pagging = pagenav($total,$page_number,$limit,$link['BEAUTASSIST']);
}
$country_code = check_user_country();
//Print Template
HtmlTemplate::display('beautassists', array(
    'pagetitle' => $Pagetitle,
    'items' => $items,
    'usersfound' => $total,
