...
if(isset($_POST['action'])){
    ...
    if ($_POST['action'] == "meetingScheduleDataSave") {meetingScheduleDataSave();}
...
function save_gig_overview()
{
    ...
    if (!count($errors) > 0) {
        ...
        add_post_customField_data($cat_id, $sub_catid, $post_id,'service');
...
function ajaxlogin(){
    ...
    {
        echo __("Your email has not been verified yet. Please verify now.");
    }
...
function write_rating(){
    ...
    if($valid_check_stamp){
        ...
        if($post_type == "service"){
            if($_SESSION['user']['user_type'] == 'beautisian'){
                $where_array = array(
                    'o.status' => 'completed',
                    'p.user_id' => $_SESSION['user']['id']
                );
