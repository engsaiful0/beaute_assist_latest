...
function userlogin($email, $password)
{
    ...
    $num_rows = ORM::for_table($config['db']['pre'].'user')
        ->select_many('id', 'status','email_verify', 'username', 'password_hash')
        ->where($where, $email)
        ->count();

    if ($num_rows >= 1) {
        $info = ORM::for_table($config['db']['pre'].'user')
            ->select_many('id', 'status','email_verify', 'username', 'password_hash', 'user_type')
            ->where($where, $email)
            ->find_one();
        ...
        $email_verify=$info['email_verify'];
        ...
        if (password_verify($password, $db_password)) {
            ...
            $userinfo['email_verify'] = $email_verify;
...
function get_user_data($username=null, $userid=null){
    ...
    if (isset($info['id'])) {
        $userinfo['id']         = $info['id'];
        $userinfo['hourly_rate']= $info['hourly_rate'];
