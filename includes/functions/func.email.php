 ...
 */
function subscriber_email($user_email)
{
   
        $html = 'Subscriber Confirmation';
        $email_subject = $html;
        $html = 'Your email has been recorded to our newsletter for regular updates about Offers, Coupons & more.';
        $email_body = $html;
        $result = email($user_email, $user_email, $email_subject, $email_body);
        return;
...
function email_template($template, $user_id = null, $password = null, $product_id = null, $item_title = null)
{
    ...
    if ($template == "signup_details") {
        ...
        $result = email($user_email, $user_fullname, $email_subject, $email_body);
