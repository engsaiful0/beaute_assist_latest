...
function gig_averageRating($post_id)
{
    ...
    $array = array('project_id' => $post_id, 'post_type' => 'service', 'rated_by' => 'user');
...
function generate_purchase_code(){
    ...
    do {
        ...
        $license_string = strtoupper(uniqid());
    } while (
        ORM::for_table($config['db']['pre'] . 'orders')
            ->where('purchase_code', $license_string)
            ->count()
    );
