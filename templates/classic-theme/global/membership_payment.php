    ...
    <div class="row">
        <div class="col-xl-8 col-lg-8 content-right-offset">
            <form id="subscribeForm" method="POST" novalidate="novalidate">
                ...
                <input type="hidden" value="0" id="amount_after_promo_code" name="amount_after_promo_code"  />
            ...
            <div class="boxed-widget summary margin-top-0">
                ...
                <div class="boxed-widget-inner">
                    <ul>
                        ...
                        <li class="total-costs"><?php _e("Promo Code") ?>
                            <form id="promo_code_apply_form">
                                <input placeholder="Enter Promo Code" name="promo_code" id="promo_code">
                                <input type="hidden" value="<?php echo $amount_without_dollar_sign ?>" name="total_cost" id="total_cost">
                                <button onclick="promo_code_apply()" type="button" name="promo_code_button" class="button big ripple-effect margin-top-40 margin-bottom-65 subscribeNow" id="promo_code_button"><?php _e("Apply") ?></button>
                            </form>
                        </li>
                        <li class="total-costs"><?php _e("Total Cost") ?> <span id="total_cost_show"><?php _esc($amount) ?></span></li>
                    </ul>
                </div>
            </div>
        </div>
...
<script type="text/javascript">
    function promo_code_apply() {
        ...
        $("#loaderIcon").show();
        var data = $('#promo_code_apply_form').serialize();
        jQuery.ajax({
            url: "<?php _esc($config['app_url']) ?>promo_code_apply.php",
            data: data,
            type: "POST",
            success: function(data) {
                if (data != 0) {
                    Snackbar.show({
                        text: 'Promo Code has been applied successfully.'
                    });
                    $data_split=data.split('_');
                    $('#total_cost').val($data_split[0]);
                    $('#amount_after_promo_code').val($data_split[0]);
                    $('#discount_amount').val($data_split[1]);
                    $('#total_cost_show').html('$'+$data_split[0]);
                } else {
                    Snackbar.show({
    ...
    $(document).ready(function() {
        ...
        function payWithStripe() {
            ...
            Stripe.card.createToken(ccData, function(status, response) {
                if (response.error) {
                    /* Visual feedback */
                    $form.find('#subscribeNow').html(LANG_TRY_AGAIN).prop('disabled', false);
