...
if(isset($_GET['action'])){
    ...
    if ($_GET['action'] == "addPromoCode") { addPromoCode(); }
    if ($_GET['action'] == "editPromoCode") { editPromoCode(); }
...
function addMembershipPlan()
{
    ...
    if (isset($_POST['submit'])) {
        ...
        $service_commission_to_admin = isset($_POST['service_commission_to_admin']) ? $_POST['service_commission_to_admin'] : 0;
  
        $skills_limit = isset($_POST['skills_limit']) ? $_POST['skills_limit'] : 0;

        $service_posting_limit = isset($_POST['service_posting_limit']) ? $_POST['service_posting_limit'] : 0;
        $service_expire_duration = isset($_POST['service_expire_duration']) ? $_POST['service_expire_duration'] : 0;
        $number_of_meeting_schedules = isset($_POST['number_of_meeting_schedules']) ? $_POST['number_of_meeting_schedules'] : 0;

        $featured_badge_duration = isset($_POST['featured_badge_duration']) ? $_POST['featured_badge_duration'] : 0;
        $urgent_badge_duration = isset($_POST['urgent_badge_duration']) ? $_POST['urgent_badge_duration'] : 0;
        ...
        $insert_subscription->service_commission_to_admin = validate_input($service_commission_to_admin);
        $insert_subscription->skills_limit = validate_input($skills_limit);
        $insert_subscription->service_posting_limit =validate_input($service_posting_limit);
        $insert_subscription->service_expire_duration = validate_input($service_expire_duration);
        $insert_subscription->number_of_meeting_schedules = validate_input($number_of_meeting_schedules);
        $insert_subscription->featured_badge_duration = validate_input($featured_badge_duration);
        $insert_subscription->urgent_badge_duration = validate_input($urgent_badge_duration);
        $insert_subscription->status = validate_input($active);
...
function addPromoCode()
{
    ...
    if (isset($_POST['submit'])) {
        ...
        $promo_code = ORM::for_table($config['db']['pre'].'promo_codes')->create();
        $promo_code->code = validate_input($_POST['code']);
        $promo_code->type = validate_input($_POST['type']);
        $promo_code->amount = validate_input($_POST['amount']);
        $promo_code->expire_date = date('Y-m-d',strtotime($_POST['expire_date']));
        $promo_code->save();
...
function editPromoCode()
{
    ...
    if (isset($_POST['submit'])) {
        ...
        $promo_code = ORM::for_table($config['db']['pre'].'promo_codes')->find_one($_POST['id']);
        $promo_code->code = validate_input($_POST['code']);
        $promo_code->type = validate_input($_POST['type']);
        $promo_code->amount = validate_input($_POST['amount']);
        $promo_code->expire_date = date('Y-m-d',strtotime($_POST['expire_date']));
