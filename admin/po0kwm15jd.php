...
if(isset($_GET['action'])){
    ...
    if ($_GET['action'] == "deletePromoCodes") { deletePromoCodes(); }
...
function deletePromoCodes()
{
    ...
    if (is_array($_POST['list'])) {
        ...
        if(check_allow()){
            ORM::for_table($config['db']['pre'].'promo_codes')->where_id_in($_POST['list'])->delete_many();
        }
...
if(isset($_POST['action'])){
    ...
    if ($_POST['action'] == "approve_post") { approve_post(); }
...
function addSubCat()
{
    ...
    if (trim($name) != '' && is_string($name) && trim($cat_id) != '') {
        ...
        $query = "Insert into `".$config['db']['pre']."catagory_sub` set sub_cat_name='".$name."',post_type='".$post_type."', slug='".$slug."', main_cat_id='".$cat_id."'";
        if(check_allow()){
            $con->query($query);
            $id = $con->insert_id;
